...
const areaOfCylinder = (r, h) => 2 * Math.PI * r * (r + h);
...
const textParagraph = (text) => document.write(`<p>${text}</p>`);
...
const unorderedListWithSize = (text, size) => {
    document.write(`<ul>`);
...
const unorderedListWithArray = (array) => {
    document.write(`<ul>`);
    for (const arrayElement of array) {
...
const printUsers = (users) => {
    ...
    for (const user of users) {
        document.write(`<div style="flex: 50%; background-color: darkcyan;">
